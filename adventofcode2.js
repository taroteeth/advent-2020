...
// high altitude overview: 
    // split the string into comma delimited array of each line
    // define correct password counter (correctpw), set to 0
    // iterate through at each line: 
        // get min and max count params
        // get letter param
        // look at rest of array item to apply params to pw
        // if pw passes param test, increase correctpw by 1
    // return correctpw 


// pseudocode and js: 

const validPwCount = (passwordString) => {
    // create pwArray by splitting input on new line
    let pwArray = passwordString.split('\n');
    // create correctpw var and set to 0
    let correctPwCount = 0;

    // iterate over pwArray item by item 
    for(let i = 0; i < pwArray.length; i++) {
        // delcare array item 
        let string = pwArray[i];
        // declare min var = []
        let minCount = [];
        // decalre max var = []
        let maxCount = [];
        // declare letter var 
        let letter = '';
        // set minFinished boolean to false
        let minFinished = false;
        // set maxfinished boolean to false
        let maxFinished = false;
        // set letterfinished boolean to false 
        let letterFinished = false;
        // define letter count as 0
        let letterCount = 0;

        // iterate over pwArray[i] 
        for(let j = 0; j < string.length; j++){
            // define array item i as char
            let char = string[j];

            // if min finished false
            if(!minFinished){
                // if array item is hyphen
                if(char === '-'){
                    // set minfinished to true 
                    minFinished = true;
                } 
                // otherwise 
                else {
                    // push character to minfinished 
                    minCount.push(char);
                }
            } 

            // if max finished is false 
            else if(!maxFinished){
                // if array item is space
                if(char === ' '){
                    // set maxfinished to true
                    maxFinished = true;
                } 
                // otherwise
                else {
                    // push character to maxfinished 
                    maxCount.push(char);
                }
            }

            // if letter finished is false
            else if(!letterFinished){
                // if array item is colon 
                if(char === ':'){
                    // set letter finished to true
                    letterFinished = true;
                } 
                // otherwise
                else {
                    // set letter to current char
                    letter = char;
                }
            } 
            // THENNNNNNN 
        ...
        let minFloor = parseInt(minCount.join(''));
        let maxCeil = parseInt(maxCount.join(''));
        // synthesize the data we got for that array item
        if(letterCount >= minFloor && letterCount <= maxCeil){
            // increment correct password var
            correctPwCount++;
        }
    } // endfor (i)
    //return correct password var
    return correctPwCount;
}

console.log(validPwCount(pw));


/// PART TWO!!!!!!! 
const passwordBoogaloo = (passwordString) => {
    // create pwArray by splitting input on new line
    let pwArray = passwordString.split('\n');
    // create correctpw var and set to 0
    let correctPwCount = 0;
    ...
    for(let i = 0; i < pwArray.length; i++) {
        ...
        let letter = '';
        // set minFinished boolean to false
        let firstPosFinished = false;
        // set maxfinished boolean to false
        let secondPosFinished = false;
        // set letterfinished boolean to false 
        let letterFinished = false;
        // declare password array to isolate input we are testing
        let miniArray = [];
        ...
        for(let j = 0; j < string.length; j++){
            ...
            // if max finished is false 
            else if(!secondPosFinished){
                // if array item is space
                if(char === ' '){
                    // set maxfinished to true
                    secondPosFinished = true;
                } 
                // otherwise
                else {
                    // push character to maxfinished 
            ...
            // now grab the rest of the letters!
            else {
                if(char !== ' '){
                    miniArray.push(char);
                }
        ...
        let positionOne = parseInt(firstPos.join(''));
        let positionTwo = parseInt(secondPos.join(''));
        let miniString = miniArray.join('');
        let indexOne = positionOne - 1;
        let indexTwo = positionTwo - 1;
        // check to see what letter is in each position
        let firstChar = miniString[indexOne];
        let secondChar = miniString[indexTwo];

        //check to see if the first and second characters match the letter
        if(firstChar === letter && secondChar !== letter){
            correctPwCount++;
        }
        else if(firstChar !== letter && secondChar === letter){
            correctPwCount++;
        }
    } // endfor (i)
...
console.log(passwordBoogaloo(pw));